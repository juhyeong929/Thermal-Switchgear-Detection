...
def main():
    ...
    for stem, rec in index.items():
        ...
        # 상간 비교: 같은 프레임의 동종 부품 중앙값을 기준으로 삼는다
        by_cls = {}
        for d in dets:
        ...
        for d in dets:
            ...
            if len(same) >= 2:
                d["dT_phase"] = round(d["t_p99"] - float(np.median(same)), 1)
            else:
                d["dT_phase"] = None
            d["verdict"], d["basis"] = judge(d["dT_phase"], d["dT_amb"])
        ...
        rows.append({"stem": stem, "panel": rec["panel"], "t_ref": round(ref, 1),
                     "n_det": len(dets),
