...
```
· 인입선로 접촉부   주의(CAUTION) · 단위 UNKNOWN 유지 — DEC 미정 (아래 참조)
· 기존 케이블헤드 라벨의 class_id 13
· 정본 4,177박스 · 1차 시험 회수분
· 나머지 5종의 단위 UNKNOWN (NQ-13 본체)
```

### 함께 검토했으나 **보류한** 것 — 인입선로 접촉부

새 PDF p13·p14 는 인입선로 접촉부를 PF&PT 반 목록에 넣었고, 이전 판의 `보류` 표기가
없다. 그러나 **새 판은 가공여부 기호(O/!/X) 체계 자체를 없앴다.** 따라서

```
(a) 보류를 해제한 것        인지
(b) 보류 구분 자체가 사라진 것    인지
```

목록 포함만으로는 가릴 수 없다. 문서상 목록 포함을 근거로 상태를 승격시키면 나중에
발주처 해석과 충돌한다. **발주처 확인 전까지 `CAUTION` · `UNKNOWN` 을 유지한다.**
