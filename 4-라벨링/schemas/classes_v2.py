...
CLASSES = [
    ...
    ThermalClass(13, "branch_contact", "분기 접촉부", LABEL, "",
                 notes="PF&PT 반 전용 명칭"),
...
PANEL_CLASSES = {
    ...
    "P5-PF&PT반":     ["silencer_power_fuse", "mold_pt", "branch_contact",
                       "incoming_contact"],
...
ANNOTATION_UNIT = {
    ...
    # 참고 P6 159박스 + 표본 15장 육안 판정 (2026-09-02)
...
ANNOTATION_UNIT_BASIS = {
    ...
    "branch_contact":         "정본·참고 모두 라벨 실적 0건 (NQ-13)",
    ...
    # 여기서는 단위를 UNKNOWN 으로 유지하고 근거만 정확히 적는다.
    # 실측치는 reports/data_audit/class_evidence.csv 가 스크립트로 집계한다.
    "vcb_contact":            "참고 P6 159박스 존재 · 단위 미판정 — 전문가/사람 판정 대기 (NQ-13)",
...
def labelable_classes():
    """라벨 대상 클래스 (제외 3종을 뺀 것). class_id 순."""
    return [c for c in sorted(CLASSES, key=lambda x: x.class_id)
            if c.label_status != EXCLUDE]
...
def labelable(panel):
    ...
    **배포용이 아니다.** 라벨러에게 보여줄 목록은 deployable() 을 쓴다.
    ...
    """
    return [c for c in PANEL_CLASSES[panel] if c not in EXCLUDED]
...
if __name__ == "__main__":
    ...
    n = {s: sum(1 for c in CLASSES if c.label_status == s)
         for s in (LABEL, CAUTION, EXCLUDE)}
    print(f"가이드 {GUIDE_VERSION} ({GUIDE_REVISED}) — 클래스 {len(CLASSES)}개  "
          f"가공 {n[LABEL]} 주의 {n[CAUTION]} 제외 {n[EXCLUDE]}\n")
